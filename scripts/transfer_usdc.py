#!/usr/bin/env python3
"""Withdraw USDC from Bottie Gnosis Safe via Polymarket bridge relay.
...
SAFE_ADDR = "0x89dcA91b49AfB7bEfb953a7658a1B83fC7Ab8F42"
...
def get_relay_deposit_address(recipient_addr):
    """Call PM bridge API to get relay deposit address for Polygon USDC withdrawal."""
    payload = json.dumps({
        "address": SAFE_ADDR,
...
def send_usdc_from_safe(w3, account, deposit_addr, amount_raw):
    """Send USDC.e from Gnosis Safe to relay deposit address via execTransaction."""
    ...
    # Build inner USDC.e transfer calldata: transfer(deposit_addr, amount)
...
def main():
    ...
    # Check USDC.e balance on the Safe
    ...
    # Step 1: Get relay deposit address from PM bridge API
    try:
    ...
    # Step 2: Send USDC.e from Safe to relay deposit address
    try:
