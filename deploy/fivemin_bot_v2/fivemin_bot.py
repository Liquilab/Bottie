...
# v4 2026-04-18 PM: per-coin strategy (price-tier + total stake per side).
# Basis: HV 30d data — BTC is multi-tier (48/29/23), alts mostly 1c.
# User directive: BTC ladder back, alts @ 1c only, DOGE/BNB/HYPE test.
# BTC split 40/30/30 (was HV 48/29/23, simpler balanced).
COIN_STRATEGIES = {
    # v9 2026-04-19 05:00: regime-shift analyse laatste 14d HV data:
    # SOL +41% → -19.6% (collapse). XRP -12.9% → +16.6% (flip positief).
    # HYPE/DOGE/BNB: dead (HV stopte, geen fills laatste 14d).
    "BTC":  {"total": 15.00, "tiers": [(0.01, 0.40), (0.02, 0.30), (0.03, 0.30)]},  # recent +8.1%
    "ETH":  {"total":  9.00, "tiers": [(0.01, 8/9), (0.05, 1/9)]},  # $8@1c + $1@5c test (2026-04-19 06:05)
    "XRP":  {"total":  2.00, "tiers": [(0.01, 1.00)]},  # 4→2 (2026-04-19 06:00) — high variance
    # SOL disabled 2026-04-19 — recent 14d ROI -19.6% (was +41.6% older 14d)
}
MIN_SHARES = 5  # PM minimum

# Killswitch: stop trading if bankroll drops below this (protects capital).
KILLSWITCH_BANKROLL = 750.00

# Funder address for on-chain balance check
FUNDER = None  # set from env

# Multi-coin active (v4 2026-04-18 PM): BTC + ETH + SOL + DOGE + BNB + HYPE
# XRP removed (HV 30d ROI +0.5% flat, post-T-15 -33.8%).
COINS = {
    # v9 2026-04-19: BTC + ETH + XRP (SOL disabled, regime collapse)
    "bitcoin":  "BTC",
    "ethereum": "ETH",
    "ripple":   "XRP",
    "xrp":      "XRP",
...
COIN_SLUGS = {
    ...
    "ETH":  "eth-updown-5m",
    "XRP":  "xrp-updown-5m",
    # SOL disabled 2026-04-19 (recent 14d ROI -19.6%)
}
...
def place_orders(client, window: Window, shares: int) -> bool:
    ...
    # Killswitch: stop trading when bankroll dips below threshold (2026-04-18)
    if bankroll > 0 and bankroll < KILLSWITCH_BANKROLL:
        log.warning(f"  🛑 KILLSWITCH — bankroll ${bankroll:.2f} < ${KILLSWITCH_BANKROLL:.2f}, skipping orders")
        return False

    strategy = COIN_STRATEGIES.get(window.coin)
    if not strategy:
        log.warning(f"  SKIP — no strategy for {window.coin}")
        return False
    budget_per_side = strategy["total"]
    ...
    n_orders = 2 * len(tiers)
    log.info(
        f"  TOTAL: {n_orders} orders ({len(tiers)} tier × 2 sides) {window.coin} | "
...
def main():
    ...
    log.info("Per-coin strategy:")
    for coin, strat in COIN_STRATEGIES.items():
        tier_str = "+".join(f"{int(pct*100)}%@{int(pr*100)}c" for pr, pct in strat["tiers"])
        log.info(f"  {coin:<5} ${strat['total']:>5.2f}/side  {tier_str}")
    log.info(f"Killswitch bankroll: ${KILLSWITCH_BANKROLL:.2f}")
    log.info(f"Min shares: {MIN_SHARES}")
    log.info(f"Coins: {', '.join(sorted(set(COINS.values())))}")
    log.info("=" * 60)
    ...
    while True:
        try:
            ...
            for cid, w in list(active_windows.items()):
                ...
                # Per-coin T-15 cancel policy (2026-04-19):
                # HV 30d post-T15 data shows +$4,742 BTC / +$2,926 ETH / +$1,720 SOL
                # (ROI +11% / +47% / +34%). ETH+SOL: cancel kost edge, laten lopen.
                # BTC: kleinere marginale edge + hogere variance, blijf cancellen.
                T15_CANCEL_COINS = {"BTC"}  # alleen deze coins cancellen we op T-15
                if w.orders_placed and not w.resolved and 0 < secs_to_end <= 15:
                    if w.coin in T15_CANCEL_COINS:
